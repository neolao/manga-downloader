...
class Downlader {
    ...
    *downloadNewChapter() {
        for (var mangaId in this.mangas) {
            var nextChapter = yield this.getNextChapter(mangaId);
    ...
    *getNextChapter(mangaId) {
        ...
        try {
            var chapters = yield _coFs2.default.readdir(mangaPath);
            ...
            var pages = yield _coFs2.default.readdir(chapterPath);
            if (pages.length === 0) {
